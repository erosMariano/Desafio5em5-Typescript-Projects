const input = document.querySelector(
	"body.page-product-single .product-info__form-input"
);
...
if (window.innerWidth <= 767) {
	if (divInfoMkt && document.querySelector(".product-info__flex-container")) {
		...
		document
			.querySelector(".product-info__flex-container")
			.insertAdjacentHTML("beforebegin", newDivInfoMkt);
	}
}
...
const valuesJuros = document.querySelectorAll(".product-info__installments")[0]
	.children;
Array.from(valuesJuros).forEach((element, index) => {
	...
	const cartao2 = `<img src="https://img.camicado.com.br/banner/icons/ic-pix.svg"> <strong>${newDesconto}</strong>`;
...
if (
	document.querySelectorAll(
		"body.page-product-single .product-info__btn--one-click"
	)[0]
) {
	document
		.querySelectorAll(
		...
		.insertAdjacentHTML("afterend", conteudoSeguranca);
}
